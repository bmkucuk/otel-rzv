...
def api_gosterge():
    ...
    kon  = conn.execute("SELECT COALESCE(SUM(tutar),0) FROM yevmiye WHERE yil=? AND islem_tipi IN ('Konaklama Geliri','Kapora Yanması') ", (yil,)).fetchone()[0] or 0
    kon   = conn.execute("SELECT COALESCE(SUM(tutar),0) FROM yevmiye WHERE yil=? AND islem_tipi IN ('Konaklama Geliri','Kapora Yanması') ", (yil,)).fetchone()[0] or 0
    ...
    for i, ay_adi in enumerate(AYLAR):
        ay = i + 1
        leo  = conn2.execute("SELECT COALESCE(SUM(tutar),0) FROM yevmiye WHERE yil=? AND ay=? AND islem_tipi IN ('Konaklama Geliri','Kapora Yanması') ", (yil,ay)).fetchone()[0] or 0
        cv_k = conn2.execute("SELECT COALESCE(SUM(tutar),0) FROM yevmiye WHERE yil=? AND ay=? AND islem_tipi IN ('Konaklama Geliri','Kapora Yanması') ", (yil,ay)).fetchone()[0] or 0
    ...
    return jsonify({
        'kartlar': {
            'kon': kon, 'kon': kon, 'restoran': restoran,
    ...
    try:
        ...
        if yev:
            ...
            if 'Tahsilat' in islem and tutar > 0 and foy_no:
                ...
                # Adisyon no varsa adisyon tablosundan da sil
                ma = _re.search(r'Adis#(\d+)', aciklama)
                if ma:
                    adis_no = int(ma.group(1))
                ...
                # Konaklama geliri silinince rezervasyon silinir
                oc = _sq.connect('/data/otel.db')
                ...
                # Kapora silinince rezervasyon tablosunu güncelle
                oc = _sq.connect('/data/otel.db')
